...
public class GwapPropertyRangeVerification extends AbstractOWLObjectPropertyViewComponent{
	    	...
	    	public void actionPerformed(ActionEvent e){
	    		...
	    		if (goldUnitsCheckBox.isSelected()){
		    		...
		    		//chooser.setFileFilter(filter);
		    	     int option = chooser.showOpenDialog(GwapPropertyRangeVerification.this);
	    	...
	    	public void actionPerformed(ActionEvent e) {
	    		...
	    		if (cAllObjectProperties.isSelected() == true) {	    				    			
    				for (OWLObjectProperty op : ont.getObjectPropertiesInSignature()) {	    					
						// only if there isn't already a validation for this property in progress and 
						// the object property isn't labeled as "relation"
						if (GwapValidation.isValidating(op, ont, label,  6) == false && 
								!getOWLModelManager().getRendering(op).equals("relation")) {
							owlObjectProperties.add(op);				
						}						    					
    				}
				...
				for (OWLObjectProperty op : owlObjectProperties) {
					...
					OWLClassExpression ce = op.getRanges(ont).iterator().next();
					// only the first domain if there is one
					ArrayList<String> currentdomainrange = new ArrayList<String>();
					currentdomainrange.add("");
				...
				if (objectAndDataProperties.size() > 0) {
	    			...
	    			if (crowdflower == true && owlObjectProperties.size() >= 5) {
		    			...
		    			// create job
	    				jobid = gwap.createJob(6, rCrowdflower.isSelected(), fOntologyDomain.getText(), objectAndDataPropertiesStr, domainrange, null, fInfo.getText(),
	    						numberOfJudgementsDesired, paymentPerAssignment);

	    				// if job creation was successful
	    				if (jobid != -1 && jobid != -2) {		    					    					
	    					System.out.print("Setting the comments in the ontology now for validation....");
	    					...
	    					for (OWLNamedObject op : objectAndDataProperties) {
	    						int increment = 0;
	    						String range = "false";
	    						System.out.println("checking !domainrange.get(i - 1).get(1).equals() for this: "+ domainrange);
	    						System.out.println(domainrange.get(i - 1).get(1));
	    						if (!domainrange.get(i - 1).get(1).equals("")) {
				    	    		...
				    	    		comment = df.getOWLAnnotation(df.getRDFSComment(), df.getOWLLiteral("#gwappropertyrange:domain=" + fOntologyDomain.getText() + 
				    	    									  ";subject=false" + ";object=" + range + ";info=" + fInfo.getText() + 
	...
	protected OWLObjectProperty updateView(OWLObjectProperty selectedProperty) {	
		...
		if (obj != null) {
			...
			fOntologyDomain.setText(GwapValidation.getDomain(obj, ont, label, 6));
			...
			fInfo.setText(GwapValidation.getInfo(obj, ont, label, 6));
			rCrowdflower.setSelected(GwapValidation.getCrowdflower(obj, ont, label, 6));
			rQuiz.setSelected(!GwapValidation.getCrowdflower(obj, ont, label, 6));
			if (rQuiz.isSelected()) {
    			bCosts.setEnabled(false);
    		}
			...
			if (GwapValidation.isValidating(obj, ont, label, 6) == true) {
				...
				results = gwap.getResults(GwapValidation.getJobID(obj, ont, label, 6));
	    		// set index for result array depending on the task-id
	    		index = (GwapValidation.getTaskID(obj, ont, label, 6) - 1) * 3;

...
public class GwapCommunication {
    ...
    public long createJob(int validationType, boolean crowdflower, String domain, ArrayList<String> subjects, 
			  ...
			  ArrayList<String> oprops, String info){
		
    	long returnValue = this.createJob(validationType, crowdflower, domain, subjects, objects, oprops, info, null);
    ...
    public long createJob(int validationType, boolean crowdflower, String domain, ArrayList<String> subjects, 
						  ArrayList<ArrayList<String>> objects, /*String property, String subject, String object,*/ 
						  ArrayList<String> oprops, String info, String goldQuestionFile) {
		...
		switch (validationType) {
			case 1: jobname		= "Check word relevance  for a domain (" + domain +")";
					...
					break;										
			case 2: jobname		= "Verify that a term is more specific than another";
			...
			case 3: jobname		= "Verify that an individual has the correct class type";
		...
		json   = "{\"key\":\"" + apikey + "\",\"task_title\":\"" + jobname + "\", \"task_type\":\"classification\", " + 
				 ...
				 "\"ts_multiple_choice\":\"false\"" + payment + "}";
				...
				switch (validationType) {
					case 1:
						...
						for (int i = 0; i < subjects.size(); i++) {
							...
							writer.append("Is the concept " + "'" + subjects.get(i) + "'" + " relevant for the domain of " + domain + "?");
						...
						// adding the gold Unit questions if desired: 
      					if (goldQuestionFile != null){
      						...
      						Scanner scanner = new Scanner(new File(goldQuestionFile));
					        ...
					        //Do not forget to close the scanner 
					         System.out.println("The writer object looks like this: "+ writer.toString());
					        scanner.close();
      					}
					...
					case 2:
						...
						for (int i = 0; i < subjects.size(); i++) {						
							for (int j = 0; j < objects.get(i).size(); j++) {
								...
								writer.append("In the domain of " + domain + ": Is class " + "'"+ subjects.get(i) + "'" + " a subclass of " + objects.get(i).get(j) + "?");
								//if (!info.equals("")) {
					...
					case 3:
						...
						for (int i = 0; i < subjects.size(); i++) {						
							for (int j = 0; j < objects.get(i).size(); j++) {
								...
								writer.append("In the domain of " + domain + ": Is " + "'" + subjects.get(i) + "'" + " a type of " + objects.get(i).get(j) + "?");
								//if (!info.equals("")) {
								//	writer.append(" Additional information: " + info);
								//}
							}							
						...
						break;
					case 4:
						// text for property validation
						for (int i = 0; i < subjects.size(); i++) {
							if (!objects.get(i).get(0).equals("")) {
								if (i != 0) {
									writer.append("\n");
								}
								writer.append("In the domain of " + domain + ": Is the subject of property " + subjects.get(i) + " of type " + objects.get(i).get(0) + "?");
								if (!info.equals("")) {
									writer.append(" Additional information: " + info);
								}
							}						
							if (!objects.get(i).get(1).equals("")) {
								// new line if there was also a subject validation
								if (!objects.get(i).get(0).equals("")) {
									writer.append("\n");
								}
								writer.append("In the domain of " + domain + ": Is the object of property " + subjects.get(i) + " of type " + objects.get(i).get(1) + "?");
								if (!info.equals("")) {
									writer.append(" Additional information: " + info);
								}
							}						

...
\item[6. Result Presentation and Interpretation.] As soon as available, the plugin presents the results to the user and saves them in the ontology. Depending on the result, the user will perform further actions such as deleting parts of the ontology which have been validated as non-relevant.
\end{description}
...
and a \texttt{GO} button to start the validation process. \\


%After enough HC users given their input on the task, the results are sent back to the plugin, which then displays them to the ProtÃ©gÃ© user. 
...
For every task, the plugin contains a predefined task description (typically including examples) which is presented to the crowd-worker.
If the ontology engineer wants to extend this task description, (s)he can provide more guidelines in the \emph{additional information} field.
...
are verified (recursively). To verify the entire ontology, the user invokes the plugin from the uppermost class, i.e., (\emph{Thing}).
...
Figure~\ref{fig:screen_cr} shows the UI for the class ``fracking'' before initiating the verification. [TBD - how does the returned result look like? Do we have a screenshot?]
...
\subsection{Crowdsourcing Task Interfaces}

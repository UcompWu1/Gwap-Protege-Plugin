...
public class GwapCommunication {
	...
	public long createJob(int validationType, boolean crowdflower, String domain, ArrayList<String> subjects, 
		...
		switch (validationType) {
			case 1: jobname		= "Check word relevance for a domain";
					description = "Please decide whether the given word (also known as concept) is relevant for the mentioned domain. " +
								  ...
								  " (for example, checking the definition of climate related terms on Wikipedia).";
					break;										
				...
				switch (validationType) {
					case 1:
						...
						for (int i = 0; i < subjects.size(); i++) {
							...
							writer.append("Is the concept " + subjects.get(i) + " relevant for the domain " + domain + "?");
							if (!info.equals("")) {
					...
					case 3:
						...
						for (int i = 0; i < subjects.size(); i++) {						
							for (int j = 0; j < objects.get(i).size(); j++) {
								...
								writer.append("In the domain of " + domain + ": Is individual " + subjects.get(i) + " an instance of class " + objects.get(i).get(j) + "?");
								if (!info.equals("")) {
									writer.append(" Additional information: " + info);
								}
							}							
